...
urls = [
'https://www.uschamberfoundation.org/aid-event/coronavirus-covid-19?page=0']
...
#     donation_description = soup.find("div", {"class": "field field-name-field-pproject-description field-type-text-with-summary field-label-hidden"})
    ...
    data = {'Company': [company], 'COVID responses': [donation_description]}
...
#     df = pd.DataFrame(data, columns=['Company', 'COVID responses'])
    df_company = pd.DataFrame(company)
    df_donation_description = pd.DataFrame(donation_description)
    
    df_company.to_csv (r'/Users/joshstrupp/Documents/GitHub/path-to-pitch-2020/commerce_page_0_csvtest_company.csv', index=False, header=True)
    df_donation_description.to_csv (r'/Users/joshstrupp/Documents/GitHub/path-to-pitch-2020/commerce_page_0_csvtest_donation_description.csv', index=False, header=True)
